(() => {
    const MAPS = [
        { id: "a", label: "A", image: "assets/Escaramuza_A.jpeg", legacy: ["district"] },
        { id: "b", label: "B", image: "assets/Escaramuza_B.jpeg", legacy: ["piazza"] },
        { id: "c", label: "C", image: "assets/Escaramuza_C.jpeg", legacy: ["kasbah"] }
    ];
    ...
    const restoreState = () => {
        try {
            ...
            if (typeof data.themeRoyal === "boolean") state.themeRoyal = data.themeRoyal;
    ...
    const startSlotAnimation = () => {
        elements.slotColumns.forEach((column) => {
            ...
            column.style.setProperty("--slot-duration", `${0.55 + Math.random() * 0.35}s`);
        });
    };
    ...
    const setWinnerBackground = (mapId) => {
        ...
        if (map?.image) {
            elements.resultCard.style.setProperty("--winner-image", `url(${map.image})`);
        }
    };
    ...
    const applyBodyClasses = () => {
        ...
        if (elements.streamerExit) {
            ...
            elements.streamerExit.setAttribute("aria-hidden", String(!state.streamerMode));
        }
    };

    const syncToggles = () => {
        if (elements.noRepeatToggle) elements.noRepeatToggle.checked = state.noRepeat;
        if (elements.soundToggle) elements.soundToggle.checked = state.soundOn;
        if (elements.streamerToggle) elements.streamerToggle.checked = state.streamerMode;
    ...
    document.addEventListener("keydown", (event) => {
        ...
        if (isInteractiveElement(document.activeElement) && document.activeElement !== document.body) return;
        event.preventDefault();
        randomize();
    });
